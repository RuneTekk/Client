...
public class AnimSequence {
    ...
    public static void unpackAnimSequences(ArchivePackage archive) {
        ByteBuffer buffer0 = new ByteBuffer(archive.getArchive("seq.dat", null));
        amountanimationseqs = buffer0.getUword();
    ...
    public void loadAnimSeq(boolean flag, ByteBuffer buffer0)
    {
        if(!flag)
        ...
        do
        {
            ...
            if(opcode == 1)
            {
                ...
                {
                    anIntArray353[j] = buffer0.getUword();
                    anIntArray354[j] = buffer0.getUword();
                    if(anIntArray354[j] == 65535)
                        anIntArray354[j] = -1;
                    anIntArray355[j] = buffer0.getUword();
                }

            } else
            if(opcode == 2)
                anInt356 = buffer0.getUword();
            else
            if(opcode == 3)
            {
            ...
            if(opcode == 6)
                anInt360 = buffer0.getUword();
            else
            if(opcode == 7)
                anInt361 = buffer0.getUword();
            else
            if(opcode == 8)
